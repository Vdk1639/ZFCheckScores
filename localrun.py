# 本地运行 
# 尝试保留CAS会话定时调用main.py 减少CAS登录次数
import re
import hashlib
import os
import shutil
...
# 登录
# 修改main.py中登录逻辑，一次CAS登录，获取教务系统的session后定时查询教务
# 优先尝试加载本地CAS会话
student_client = None
base_url = None
jw_ses = None
#不是GitHub Actions环境时读取配置文件
if not github_actions and os.path.exists("config.txt"):
    with open("config.txt", "r") as f:
        lines = f.readlines()
        username = lines[0].strip()  # 第一行是用户名
        password = lines[1].strip()  # 第二行是密码
        url = lines[2].strip()  # 第三行是URL
        token = lines[3].strip()  # 第四行是TOKEN
if os.path.exists(cas_session_file):
    try:
        ...
        if is_jw_session_valid(base_url, jw_ses):
            ...
            print("使用本地缓存的CAS会话，无需重新登录。")
        else:
            print("本地CAS会话已失效，重新登录CAS。")
            base_url, jw_ses = None, None
    except Exception as e:
        print(f"加载本地CAS会话失败: {e}")
        base_url, jw_ses = None, None
